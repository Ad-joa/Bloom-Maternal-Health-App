...
const blushPink = { r: 255/255, g: 228/255, b: 230/255 };
...
function createScreenFrame(name: string) {
  ...
  frame.fills = [{type: 'SOLID', color: {r: 252/255, g: 250/255, b: 248/255}}]; // Soft Background
  return frame;
}
...
function createCard(parent: FrameNode, w: number, h: number, x: number, y: number, radius: number = 24) {
...
function createTopAppBar(parent: FrameNode, title: string, showBack: boolean) {
  ...
  headerDate.x = 36; headerDate.y = 14;
...
function createBottomNav(parent: FrameNode, activeTab: number = 0) {
  ...
  nav.effects = [{
    ...
    offset: {x: 0, y: -2},
    radius: 16, spread: 0, visible: true, blendMode: 'NORMAL'
  }];
...
function createButton(parent: FrameNode, text: string, x: number, y: number, width: number, isPrimary: boolean = true) {
  ...
  btnText.x = x + (width - (text.length * 9)) / 2; // rough center
  btnText.y = y + 18;
  parent.appendChild(btnText);
  return btn;
}

function createInputField(parent: FrameNode, placeholder: string, x: number, y: number, width: number) {
...
function createProgressBar(parent: FrameNode, progress: number, x: number, y: number, width: number) {
...
function createListItem(parent: FrameNode, title: string, subtitle: string, hasIcon: boolean, x: number, y: number, width: number) {
...
function createAvatar(parent: FrameNode, radius: number, x: number, y: number) {
...
function buildSplashScreen(frame: FrameNode) {
  ...
  createProgressBar(frame, 0.6, 100, 700, 193); // A loading progress bar

  addText(frame, "v1.0.0", "Regular", 12, 175, 780, textLight);
}

function buildLandingScreen(frame: FrameNode) {
  frame.fills = [{type: 'SOLID', color: blushPink}];
...
function buildOnboardingScreen(frame: FrameNode) {
...
function buildLoginScreen(frame: FrameNode) {
  // Blush hero gradient at top
  ...
  logoMark.x = (393 - 72) / 2; logoMark.y = 100;
...
function buildRegisterScreen(frame: FrameNode) {
  // Blush hero
  const hero = figma.createRectangle();
  hero.resize(393, 240);
  hero.fills = [{type: 'SOLID', color: blushPink}];
  ...
  decCircle.x = 280; decCircle.y = -40;
...
function buildInsightsScreen(frame: FrameNode) {
...
function buildChatsScreen(frame: FrameNode) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    ...
    addText(frame, contacts[i][0] as string, "Bold", 15, 100, y + 18, textDark);
    addText(frame, contacts[i][1] as string, "Regular", 13, 100, y + 40, textLight);
  }
}

function buildTrackerScreen(frame: FrameNode) {
...
function buildHomeScreen(frame: FrameNode) {
  addText(frame, "Today, Oct 24", "Medium", 14, 24, 110, textLight);
  addText(frame, "Good Morning, Sarah 👋", "Bold", 24, 24, 130, textDark);

  // Advanced tracker
  const tracker = figma.createEllipse();
  tracker.resize(250, 250);
  tracker.x = (393 - 250) / 2; tracker.y = 190;
  tracker.fills = [{
    type: 'GRADIENT_ANGULAR',
    gradientTransform: [[1, 0, 0.5], [0, 1, 0.5]],
    gradientStops: [
  ...
  // Center text in tracker
  const trackerInner = figma.createEllipse();
  trackerInner.resize(210, 210);
  trackerInner.x = (393 - 210) / 2; trackerInner.y = 210;
...
function buildSymptomCheckerScreen(frame: FrameNode) {
...
function buildProfileScreen(frame: FrameNode) {
...
function buildTrimesterInfoScreen(frame: FrameNode) {
...
function buildDangerSignsScreen(frame: FrameNode) {
...
function buildVisitsScreen(frame: FrameNode) {
...
figma.ui.onmessage = async (msg: any) => {
  try {
    ...
    if (msg.type === 'generate-screen') {
      ...
      switch (screenType) {
        case 'SplashScreen': buildSplashScreen(frame); break;
        case 'LandingScreen': buildLandingScreen(frame); break;
        case 'OnboardingScreen': buildOnboardingScreen(frame); break;
        case 'LoginScreen': buildLoginScreen(frame); break;
        case 'RegisterScreen': buildRegisterScreen(frame); break;
        case 'HomeScreen': buildHomeScreen(frame); break;
        case 'InsightsScreen': buildInsightsScreen(frame); break;
        case 'ChatsScreen': buildChatsScreen(frame); break;
        case 'TrackerScreen': buildTrackerScreen(frame); break;
        case 'SymptomCheckerScreen': buildSymptomCheckerScreen(frame); break;
        case 'ProfileScreen': buildProfileScreen(frame); break;
        case 'TrimesterInfoScreen': buildTrimesterInfoScreen(frame); break;
        case 'DangerSignsScreen': buildDangerSignsScreen(frame); break;
        case 'VisitsScreen': buildVisitsScreen(frame); break;
      }
