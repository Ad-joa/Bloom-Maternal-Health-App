  ...
  for (const user of users) {
    ...
    if (lmpParts.length === 3) {
      ...
      if (!isNaN(lmpDate.getTime())) {
        ...
        const fixedLmp = `${monthStr}/${dayStr}/${yearStr}`;
        if (user.due_date !== isoDueDate || user.last_period_date !== fixedLmp) {
          ...
          fixedCount++;
          console.log(`Fixed user ${user.id}: LMP -> ${fixedLmp}, Due Date -> ${isoDueDate}`);
        }
      }
    }
  }
  console.log(`Finished fixing dates. Total users updated: ${fixedCount}`);
}
